-- Copied from: https://raw.githubusercontent.com/Rishabh672003/Neovim/refs/heads/main/lua/rj/extras/workspace-diagnostic.lua
...
function M.populate_workspace_diagnostics(client, bufnr)
  ...
  if not vim.tbl_get(client.config, 'filetypes') then
    local msg = '[workspace-diagnostics] ' .. client.name .. ' is skipped: please define `config.filetypes` when setting up the client.'
...
-- Optional: close diagnostics on leave after N ms to reduce memory
vim.g.workspace_diagnostic_close_after_ms = vim.g.workspace_diagnostic_close_after_ms or 500
