...
Create/edit `~/.config/tools/config` with one URL per line:
...
```bash
~/.config/dotfiles/Hooks/tools/post.sh
```

### Preview what would be installed (dry-run)

```bash
~/.config/dotfiles/Hooks/tools/post.sh --dry-run
```

### Verbose output (debug mode)

```bash
~/.config/dotfiles/Hooks/tools/post.sh --debug
```

### Disable spinner/progress animation

```bash
~/.config/dotfiles/Hooks/tools/post.sh --no-spinner
```
...
- **Error logging**: Failures logged to `$HOME/.local/var/log/tools-hook-errors.log`
...
All failures are logged to `$HOME/.local/var/log/tools-hook-errors.log` with timestamps. If any downloads fail, the script exits with code 1 after attempting all URLs.
...
```bash
bash ~/.config/dotfiles/Hooks/tools/post.sh
```

Or with options:

```bash
bash ~/.config/dotfiles/Hooks/tools/post.sh --dry-run --debug
```
